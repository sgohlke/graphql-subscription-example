...
export const userSchemaResolvers= {
    ...
    userChanged(): AsyncGenerator<{
        userChanged: User;
    }, void, unknown> {
        return userChangedGenerator()  
    }
}

async function* userChangedGenerator(): AsyncGenerator<{
    userChanged: User;
}, void, unknown> {
    await sleep(5000) 
    yield { userChanged: {userId: 'test', userName: `User changed at ${new Date()}`}  }
}
